...
interface ImageGalleryProps {
  onImageClick: (image: typeof scamImages[0]) => void;
}
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        ...
        <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
          Click on any image below to see how mAIscam uses OCR and cultural intelligence to detect scams hidden within screenshots and images
        </p>
      </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-start gap-3">
                <div className={`p-2 rounded-lg ${
                  image.category === 'Government' ? 'bg-blue-100' :
                  image.category === 'SMS' ? 'bg-green-100' :
                  image.category === 'Investment' ? 'bg-purple-100' :
                  'bg-orange-100'
                }`}>
                  <image.icon className={`h-5 w-5 ${
                    image.category === 'Government' ? 'text-blue-600' :
                    image.category === 'SMS' ? 'text-green-600' :
                    image.category === 'Investment' ? 'text-purple-600' :
                    'text-orange-600'
                  }`} />
                </div>
