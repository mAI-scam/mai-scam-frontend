  ...
  },
  {
    ...
    image: {
      ...
      scamImage: {
        ...
        category: "Personal",
        content:
          "Beautiful sunset from my balcony today! Nothing beats ending the day with such amazing colors in the sky. Hope everyone is having a great week! #sunset #malaysia #photography",
      },
    },
...
export function FacebookFeed({ onImageClick }: FacebookFeedProps) {
  return (
    <div
      className="max-w-2xl mx-auto space-y-3 md:space-y-4 px-2 md:px-0"
      data-tour="facebook-feed"
    >
      {/* Create Post Box */}
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-3 md:p-4">
        <div className="flex items-center space-x-2 md:space-x-3">
          ...
          <div className="flex-1 bg-gray-100 rounded-full px-3 md:px-4 py-2 md:py-3 text-gray-500 cursor-pointer hover:bg-gray-200 transition-colors text-sm md:text-base">
            What&apos;s on your mind?
          </div>
        </div>
      </div>
      ...
      {facebookPosts.map((post) => (
        <FacebookPost key={post.id} post={post} onImageClick={onImageClick} />
      ))}
    </div>
  );
}
